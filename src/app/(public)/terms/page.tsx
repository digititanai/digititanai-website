...
export default function TermsOfServicePage() {
  const [content, setContent] = useState(defaultPageContent.terms)

  useEffect(() => {
    fetch('/api/data/page_content_terms', { cache: 'no-store' })
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data) setContent({ ...defaultPageContent.terms, ...data }) })
      .catch(() => {})
  }, [])
        ...
        <div className="container-main">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex items-center gap-4 mb-5">
              <div className="w-12 h-12 rounded-xl bg-brand-gold/10 border border-brand-gold/20 flex items-center justify-center"><FileText className="w-5 h-5 text-brand-gold" /></div>
              <div>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex-1 min-w-0 max-w-3xl">
              {sections.map(({ id, title, html }) => (
                <div key={id} id={id} className="mb-10 scroll-mt-28">
                  {id !== 'intro' && <h2 className="text-[22px] font-display font-bold text-brand-cream mb-4">{title}
                  </h2>}
